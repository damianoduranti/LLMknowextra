...
def create_output_dir(trace_path, folder_name):
    ...
    output_dir = os.path.join("output", "LTL_process", constraint_type, folder_name)
...
def ltl_process(trace_path, nuxmv_path='NUXMV', max_attempts=5):
    """
    Generate LTL candidates until a correct LTL formula is found.

    Parameters:
        trace_path (str): Path to the JSON trace file.
        nuxmv_path (str): Path to the NUXMV executable.
        max_attempts (int): Maximum number of attempts to find a valid LTL formula.

    Returns:
        response(str): A correct LTL formula if found, None otherwise.
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        with open(result_path, 'w') as file:
            file.write(f"Attempt: {attempt}\nPrompt: {prompt}\nResponse: {response}\nF2I Response: {f2i_response if 'f2i_response' in locals() else 'None'}\nError: {error}\nSpec Path: {spec_path if spec_path else 'None'}\nEvaluated Result: {'True' if 'result' in locals() and result else 'False'}\n")
